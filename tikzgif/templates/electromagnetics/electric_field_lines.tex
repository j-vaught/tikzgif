...
% Draw charges
\filldraw[red!80!black] (\xA, \yA) circle (5pt);
\node[font=\bfseries\small, white] at (\xA, \yA) {+};

\pgfmathparse{\qsign > 0 ? 1 : 0}
\ifnum\pgfmathresult=1
    \filldraw[red!80!black] (\xB, 0) circle (5pt);
    \node[font=\bfseries\small, white] at (\xB, 0) {+};
\else
    \filldraw[blue!80!black] (\xB, 0) circle (5pt);
    \node[font=\bfseries\small, white] at (\xB, 0) {--};
\fi
...
\foreach \startAngle in {0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330} {
    ...
    \draw[orange!70!black, thin, -{Stealth[length=2pt]}] \fieldpath;
}

\draw[gray!30, thin] (-4, -3.5) rectangle (5, 3.5);
