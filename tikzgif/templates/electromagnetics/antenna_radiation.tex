...
\foreach \r in {0.875, 1.75, 2.625, 3.5} {
    \draw[gray!25, thin] (0,0) circle (\r);
}
\foreach \a in {0, 30, 60, ..., 330} {
    \draw[gray!25, thin] (0,0) -- (\a:\Rmax+0.2);
    \node[font=\tiny, gray!60] at (\a:\Rmax+0.5) {$\a^\circ$};
}

% Pattern
\draw[red!80!black, very thick, samples=360, domain=0:360, variable=\t]
    plot ({\t + \rotA}:
          {\Rmax * (abs(sin(\t)) > 0.001 ? pow(abs(sin(\t)), \nExp) : 0)});

\fill[red!20, opacity=0.3, samples=360, domain=0:360, variable=\t]
    plot ({\t + \rotA}:
          {\Rmax * (abs(sin(\t)) > 0.001 ? pow(abs(sin(\t)), \nExp) : 0)})
    -- cycle;
...
% Label
\node[anchor=south west, font=\normalsize, fill=white, inner sep=3pt, draw=gray!40]
