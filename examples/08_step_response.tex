...
% Parameter: \PARAM sweeps 0 to 1 (default range), mapped to 0-4 seconds
% Render: tikzgif render 08_step_response.tex
% Difficulty: advanced
...
\newcommand{\drawSMD}[6]{%
  \begin{scope}[shift={(#1, 0)}]
    ...
    % ── Damper (fixed-rod dashpot, cylinder anchored to wall) ──
    % Piston rod is fixed length, moves with mass.
    % Piston head slides inside cylinder.
    \pgfmathsetmacro{\cylH}{0.15}           % cylinder half-height
    \pgfmathsetmacro{\margin}{0.04}
    \pgfmathsetmacro{\cylWidth}{#6 * \dispScale + 2*\margin}  % sized for max travel
    \pgfmathsetmacro{\cylLeft}{\eqX - \massW/2 - \cylWidth}
    \pgfmathsetmacro{\cylRight}{\cylLeft + \cylWidth}
    \pgfmathsetmacro{\pistonRodLen}{\cylWidth - \margin}       % fixed rod length

    % Rod from wall to cylinder closed end (fixed length)
    ...
    % Piston head: fixed rod from mass, slides inside cylinder
    \pgfmathsetmacro{\pistonX}{\massLeft - \pistonRodLen}

    % Piston head (vertical bar inside cylinder)
    \draw[very thick, black!80]
      (\pistonX, {\damperY - \cylH + 0.02}) -- (\pistonX, {\damperY + \cylH - 0.02});

    % Piston rod from piston to mass (fixed length)
