...
\newcommand{\drawSMD}[6]{%
  \begin{scope}[shift={(#1, 0)}]
    ...
    % ── Damper (proper dashpot, cylinder sized for 1:1 piston travel) ──
    \pgfmathsetmacro{\cylH}{0.15}
    \pgfmathsetmacro{\margin}{0.04}
    % Cylinder width = full displacement range + margins
    \pgfmathsetmacro{\cylW}{#6 * \dispScale + 2*\margin}
    % Rod fills remaining space between wall and cylinder
    \pgfmathsetmacro{\rodLen}{\eqX - \massW/2 - \cylW}
    ...
    % Piston head: moves 1:1 with mass displacement
    \pgfmathsetmacro{\pistonX}{\cylLeft + \margin + #3 * \dispScale}
