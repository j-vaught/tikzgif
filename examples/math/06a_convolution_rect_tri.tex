% Example 13a: Signal Convolution (Rect * Triangle)
...
% Parameter: \PARAM (0 to 1, mapped to time shift t = -2 to 4)
% Render: tikzgif render 13a_signal_convolution.tex
% Difficulty: advanced
...
\begin{tikzpicture}
  ...
  \pgfmathsetmacro{\tshift}{\PARAM * 6 - 2}
  ...
  \begin{axis}[
    ...
    % f=1 on [0,2], g=1-|x-t| on [t-1,t+1], g<=1 always
    % So min(f,g) = g = 1-|x-t| on the overlap interval
    ...
    \ifnum\pgfmathresult=1
      \addplot[fill=honeycomb!50, opacity=0.3, draw=none,
        domain=\overlapLeft:\overlapRight, samples=50]
        {1 - abs(x - \tshift)} \closedcycle;
    \fi
  \end{axis}
