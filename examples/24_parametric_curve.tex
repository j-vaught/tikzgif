...
% Parameter: \PARAM (0 to 1, mapped to maximum t = 0.1 to 2*pi)
% Render: tikzgif render 24_parametric_curve.tex
...
\pgfmathsetmacro{\tmax}{\PARAM * 6.183185 + 0.1}
...
% Component strip: y(t) along bottom (rotated 90 CW: y mapped to horizontal)
\begin{scope}[shift={(0, -\amp-1.2)}]
    \draw[warmgrey!30, thin] (-\amp, -0.4) rectangle (\amp, 0.4);
    \draw[warmgrey!25, thin, samples=100, domain=0:\tmaxDeg, variable=\t]
        plot ({\amp * sin(\fb*\t)}, {0.35 * sin(\fb*\t)});
    \filldraw[garnet] (\currY, {0.35 * sin(\fb*\tmaxDeg)}) circle (2pt);
    \draw[dashed, warmgrey!30, thin] (\currY, 0.4) -- (\currY, {-(-\amp-0.5) - \amp - 1.2});
    \node[font=\tiny, warmgrey, anchor=west] at (\amp+0.15, 0) {$y(t)$};
\end{scope}

% Fix bounding box across all frames
\pgfresetboundingbox
\useasboundingbox (-\amp-0.8, -\amp-2.0) rectangle (\amp+2.0, \amp+2.0);
