...
% Parameter: \PARAM (0 to 1, mapped to proportional gain K = 0.1 to 10.0)
% Render: tikzgif render 18_block_diagram_gain_sweep.tex
...
\begin{document}
\begin{tikzpicture}[
    block/.style={draw, thick, minimum height=1cm, minimum width=2.2cm,
                  fill=white},
...
\pgfmathsetmacro{\K}{\PARAM * 9.9 + 0.1}
...
\begin{axis}[
    ...
    % Overshoot dashed line (underdamped)
    \ifnum\isunder>0
...
\node[sumnode] (sum) at (2,-2.7) {};
\node[above left=0.5mm and -1mm] at (sum.east) {\tiny $+$};
\node[right=0.5mm] at (sum.south) {\tiny $-$};
...
% Reset and fix bounding box
\pgfresetboundingbox
\useasboundingbox (-1.8, -5.5) rectangle (13, 4.5);
