...
% Parameter: \PARAM (frequency ratio r = w/w_n, range 0.2..5.0)
% Range: 0.2 to 5.0, recommended 60 frames at 15fps
% Difficulty: advanced
...
\pgfmathsetmacro{\animPhase}{360 * 5 * (\rNow - 0.2) / 4.8}
...
% Piston rod extends from mass down into cylinder.
\pgfmathsetmacro{\cylW}{0.18}           % cylinder half-width
\pgfmathsetmacro{\cylHeight}{1.6}       % fixed cylinder height
\pgfmathsetmacro{\cylBot}{\baseY + 0.15}
...
% Piston head: tracks relative displacement, scaled to stay inside cylinder
\pgfmathsetmacro{\devFromEq}{\yMass - \yBase}
\pgfmathsetmacro{\maxDev}{1.5}
\pgfmathsetmacro{\pistonMid}{(\cylBot + \cylTop) / 2}
\pgfmathsetmacro{\pistonTravel}{(\cylHeight / 2) - 0.12}
\pgfmathsetmacro{\pistonY}{max(min(
  \pistonMid + \devFromEq / \maxDev * \pistonTravel,
  \cylTop - 0.1), \cylBot + 0.1)}

% Piston head
\draw[very thick, black!80]
  ({3.0 - \cylW + 0.02}, \pistonY) -- ({3.0 + \cylW - 0.02}, \pistonY);

% Piston rod from mass down through open top to piston head
...
\node[font=\scriptsize, black!60, right] at (3.35, {\pistonMid}) {$c$};
...
\begin{axis}[
  ...
  % 0 dB reference line
  \addplot[dashed, warmgrey, thin, domain=0.1:10, samples=2] {0};

  % Full transmissibility magnitude curve
  \addplot[domain=0.1:10, samples=400, atlantic, very thick]
...
\begin{axis}[
  ...
  \addplot[dashed, warmgrey, thin, domain=0.1:10, samples=2] {-90};

  % Full transmissibility phase curve
  % Use safe ratio clamped to avoid pgfmath overflow
  \addplot[domain=0.1:10, samples=400, garnet, very thick]
