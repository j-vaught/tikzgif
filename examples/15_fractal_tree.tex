...
% Parameter: \PARAM (0 to 1, growth progress)
% Render: tikzgif render 15_fractal_tree.tex
% Difficulty: advanced
...
\begin{tikzpicture}
  ...
  % Always traverses the full tree so every node consumes its RNG values
  % on every frame, preventing jitter from shifting RNG sequences.
  \newcommand{\branch}[6]{%
    \pgfmathtruncatemacro{\dep}{#5}%
    %
    % BASE CASE: terminal leaf node
    \ifnum\dep=\maxdepth
      % Always consume 3 rnd values for RNG consistency
      \pgfmathsetmacro{\leafsz}{0.06 + 0.03 * rnd}%
      \pgfmathsetmacro{\leafclr}{40 + 30 * rnd}%
      \pgfmathsetmacro{\leafopa}{0.6 + 0.3 * rnd}%
      % Draw leaf only if parent depth is fully grown
      ...
      \ifdim\parentvis pt>0.5pt
        ...
        \fill[horseshoe!\leafclr!grass, opacity=\finalopa] (#1, #2) circle ({1.5*\leafsz});%
      \fi
    \else
      % Always consume 2 rand values for RNG consistency
      \pgfmathsetmacro{\angleJitter}{5 * rand}%
      \pgfmathsetmacro{\lenJitter}{1 + 0.15 * rand}%
      \pgfmathsetmacro{\adjlen}{#4 * \lenJitter}%
      %
      % Draw BEFORE computing child endpoints — #1/#2/#3 are tokens like
      % \ex/\ey/\angleL from the parent; redefining them early would make
      % the draw command resolve to the wrong (child) coordinates.
      ...
      \ifdim\branchvisible pt>0.5pt
        ...
        \draw[line width=#6 pt, line cap=round, horseshoe!\greenpct!warmgrey!60!black]
          (#1, #2) -- (\ix, \iy);%
      \fi
      %
      % Now safe to redefine \ex, \ey, etc. for child recursion
      ...
      %
      % Always recurse to maintain RNG consistency across frames
      \begingroup
        \branch{\ex}{\ey}{\angleL}{\newlen}{\newdep}{\newthick}%
      \endgroup
      \begingroup
  ...
  % Ground
  \fill[horseshoe!40!warmgrey!20] (-5, -0.3) rectangle (5, 0);
  \draw[horseshoe!50!warmgrey, thick] (-5, 0) -- (5, 0);
