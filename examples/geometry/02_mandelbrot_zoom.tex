% Example 02: Mandelbrot Zoom
% Progressive zoom into the Mandelbrot set boundary near the Seahorse Valley.
% Uses LuaLaTeX for fast pixel-level computation (320x240, 200 iterations).

% THIS WILL TAKE A LONG TIME TO RUN - the Lua code is not optimized for speed, but demonstrates the concept.

% Parameter: \PARAM sweeps 0 to 1 (default range), log-scale zoom
% Render: tikzgif render 02_mandelbrot_zoom.tex --timeout 0
% Engine: lualatex (auto-detected)
% Difficulty: advanced
...
\begin{document}
\begin{tikzpicture}
  \pgfmathsetmacro{\zoomval}{\PARAM}
  \useasboundingbox (0,0) rectangle (8,6);

  % Render Mandelbrot via Lua
  \directlua{mandelbrot_frame(\zoomval)}
