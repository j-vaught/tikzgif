...
\begin{tikzpicture}
  ...
  \begin{scope}[shift={(-5,0)}]
    ...
    \fill[garnet!40, opacity=0.6] (3.65, 1.1) rectangle (4.35, {1.1 + \fillht});
    ...
    \draw[->, garnet, line width=\linewd pt]
      (2.9, 3.6) -- (3.5, 3.6);
    \node[above, font=\tiny, garnet] at (3.2, 3.7) {$i(t)$};
  ...
  \begin{axis}[
    ...
    xmin=0, xmax=5,
    grid=major,
    grid style={warmgrey!25},
    title={\small Capacitor Charging},
  ]
    % Full response curve (faint)
    \addplot[warmgrey!40, thick] {\Vs * (1 - exp(-x))};

    % Traced portion up to current time
    \addplot[garnet, very thick, domain=0:\ttau] {\Vs * (1 - exp(-x))};
    ...
    % Asymptote
    \addplot[dashed, atlantic!50, thin] {\Vs};
    \node[right, font=\tiny, atlantic!50] at (axis cs:4.2, 5.2) {$V_s$};
