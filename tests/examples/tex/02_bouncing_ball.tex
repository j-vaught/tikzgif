...
\begin{tikzpicture}
  ...
  % Trajectory guide (faint) -- two arcs
  \draw[dashed, gray!40, thin]
    plot[domain=0:0.5, samples=40, variable=\t]
    ({\t * 20}, {4 * 4 * (2*\t) * (1 - 2*\t)});
  \draw[dashed, gray!40, thin]
    plot[domain=0.5:1, samples=40, variable=\t]
    ({\t * 20 - 10}, {4 * 4 * (2*\t - 1) * (1 - (2*\t - 1))});
  ...
  % Shadow on the ground (shrinks when ball is high)
  \pgfmathsetmacro{\shadowscale}{1 - 0.5 * \ypos / 4}
  \pgfmathsetmacro{\shadowalpha}{70 - 40 * \ypos / 4}
