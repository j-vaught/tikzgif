...
\begin{tikzpicture}
  ...
  \begin{axis}[
    ...
    \addplot[name path=f, red!70!black, very thick]
    ...
    \addplot[name path=g, blue!70!black, thick, draw=none]
      coordinates {
        (-3, 0)
        ({\tshift - 1}, 0)
        ({\tshift}, 1)
    ...
    \addplot[yellow!50!orange, opacity=0.3] fill between[of=f and g,
  ...
  \begin{axis}[
    ...
    % t in [-1, 0]:  0.5*(t+1)^2
    % t in [0, 1]:   -t^2 + t + 0.5
    % t in [1, 2]:   t^2 - 3*t + 2.5
    % ... simplified: we approximate numerically
    \addplot[gray!60, very thick, smooth] {
      (x >= -1 && x < 0) * (0.5*(x+1)^2) +
      (x >= 0  && x < 1) * (-x^2 + x + 0.5) +
      (x >= 1  && x < 2) * (x^2 - 3*x + 2.5) +
      (x >= 2  && x < 3) * (0.5*(3 - x)^2)
    ...
    \addplot[red!70!black, very thick, smooth, domain=-3:\tshift] {
      (x >= -1 && x < 0) * (0.5*(x+1)^2) +
      (x >= 0  && x < 1) * (-x^2 + x + 0.5) +
      (x >= 1  && x < 2) * (x^2 - 3*x + 2.5) +
      (x >= 2  && x < 3) * (0.5*(3 - x)^2)
    ...
    \pgfmathsetmacro{\convval}{%
      ...
      (\tshift >= 0  && \tshift < 1) * (-\tshift*\tshift + \tshift + 0.5) +
      (\tshift >= 1  && \tshift < 2) * (\tshift*\tshift - 3*\tshift + 2.5) +
    ...
    \draw[dashed, red!50, thin] (axis cs:\tshift, 0) -- (axis cs:\tshift, \convval);
